...
- Outputs:
  - `result` (object) – Map of declared function outputs (or flattened value when none declared).
- Notes: Looks up the descriptor via PipelineFunctionProvider, evaluates the referenced pipeline, resolves nested yields/returns, and collects declared outputs.
